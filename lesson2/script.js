...
const userBirthday = prompt('Enter your year of birth')
const today = new Date()

let name = document.getElementById('name')
name.innerHTML = (`Full name: ${userName.trim()}`)

let totalEmail = document.getElementById('email')
if (!userEmail.includes('@')) {
    totalEmail.innerHTML=(`Email: not valid email ${userEmail.replace(/ /g,'')} (symbol @ not exist)`)
 } else if (userEmail.startsWith('@')) {
    totalEmail.innerHTML=(`Email: not valid email ${userEmail.replace(/ /g,'')} (symbol @ find in first place)`)
 } else if (userEmail.endsWith('@')) {
    totalEmail.innerHTML=(`Email: not valid email ${userEmail.replace(/ /g,'')} (symbol @ find in last place)`)
 } else {
    totalEmail.innerHTML=(`Email: ${userEmail.replace(/ /g,'')}`)
 }

 let age = document.getElementById('age')
 age.innerHTML = (`Age: ${today.getFullYear() - userBirthday.replace(/ /g,'')}`)
